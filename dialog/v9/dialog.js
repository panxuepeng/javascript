...
setTimeout(function() {
	// 增加一个隐藏的table，用于计算内容的尺寸
	$('body').append('<div id="'+ContainerId+'"></div>')
})

// 给内容区域包裹一个table标签，以便计算内容区域的尺寸
var dialogHtml = '<div id="mod-dialog-%dialogCount%" class="mod-dialog" style="left:-2000px; width: %width%px;">'
	...
	+ '<div class="mod-dialog-content" style="height: %height%px; overflow:hidden;"><table border="0"><tr><td>%content%</td></tr></table></div>'
...
function getContentSize(html) {
	...
	td.append(html)
	var val = {width: td.width(), height: td.height()}
	td.empty()
	console.log(val)
	return val
}
